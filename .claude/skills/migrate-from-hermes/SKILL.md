---
name: migrate-from-hermes
description: Migrate from Hermes to NanoClaw. Detects existing Hermes installation at ~/.hermes, extracts identity, channel credentials, scheduled tasks, and other config, then guides interactive migration. Triggers on "migrate from hermes", "hermes migration", "import from hermes".
---
...
Before identity/memory, extract settings from `config.yaml` that map directly to NanoClaw setup. Read the config file with the Read tool (`<STATE_DIR>/config.yaml`).
...
**If HAS_CREDENTIAL=false but the user expects a credential:** The extraction script may not recognize the config structure. Fall back to reading the channel section of `config.yaml` directly with the Read tool and look for any field that contains a token or key value. Ask the user to confirm.
...
Read relevant sections from `<STATE_DIR>/config.yaml` directly with the Read tool. This phase is fully conversational.
...
**Config parse error:** If `config.yaml` fails to parse, the file may have unusual syntax. Ask the user to check the file. As a fallback, the agent can read the file directly and work with it manually.
